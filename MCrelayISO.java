...
class MCrelayISO {  
	...
	public static void main(String args[])throws Exception{  
		...
		try { p.load(new FileInputStream("MCrelayISO.ini")); } 
      		...
      		System.out.println("---------------------------------");
      		System.out.println("Puerto SibLink                    = " + p.getProperty("SLport"));
      		System.out.println("Puerto Cliente                    = " + p.getProperty("CLIport"));
      		System.out.println("TimeOut para enviar ECO a SibLink = " + p.getProperty("SLECOsendtimeout"));
      		System.out.println("TimeOut para Recibir respuesta ECO= " + p.getProperty("SLECOresptimeout"));
      		System.out.println("Loguea ECO                        = " + p.getProperty("LOGecos"));
      		System.out.println("Loguea Datos                      = " + p.getProperty("LOGdata"));
      		System.out.println("Loguea Conexiones                 = " + p.getProperty("LOGconn"));
      		System.out.println("Tamaño de bloque leido            = " + p.getProperty("MSGblocksize"));
      		System.out.println("Esperar 8 estaciones al inicio    = " + p.getProperty("EsperaEstaciones"));
      		System.out.println("Debug sobre trama CLiente         = " + p.getProperty("DebugTramaCliente"));
      		System.out.println("Debug sobre trama Servidor        = " + p.getProperty("DebugTramaServidor"));
      		System.out.println("---------------------------------");
      		System.out.println(" ");
      		System.out.println(" ");
		...
		MCrelayISO.SLECOsendtimeout=Integer.parseInt(p.getProperty("SLECOsendtimeout"));
		MCrelayISO.SLECOresptimeout=Integer.parseInt(p.getProperty("SLECOresptimeout"));
		...
		for (int poolid=0; poolid<MaxConnections; poolid++) MCrelayISO.ConnPool[poolid]=-1; 
		for (int poolid=0; poolid<9; poolid++) MCrelayISO.SLFreePool[poolid]=-1; 
		...
		try {  
		    	...
		    	ts.start(); 
				} catch (Exception e) { System.out.println("ATENCION: Error al abrir el puerto ("+MCrelayISO.SLport+"), Es posible que el Relay IP Siblink ya este iniciado ...\n\n"); System.exit(2);}   
		...
		try {
			...
			tc.start(); 
				} catch (Exception e) { System.out.println("ATENCION: Error al abrirel puerto ("+MCrelayISO.CLIport+")  Es posible que el Relay IP Siblink ya este iniciado ...\n\n"); System.exit(2);}
...
public static void MuestraConn() {
        ...
        System.out.println("\n|-- SLNK - CLI --|");
          System.out.println("|--------+-------|");
        for (int p=0; p<8; p++) {
	   System.out.println("|-> "+String.format("%04d",MCrelayISO.SLFreePool[p])+" + "+String.format("%04d",MCrelayISO.ConnPool[p])+" -| ip: "+MCrelayISO.ConnIpPool[p]); }
         System.out.println(" ");
	}
	...
	MCrelayISO.SLconnID=0;
        ...
        while (Mainexit)  {
            ...
            try // Block 1
            {
               ...
               for (int poolid=0;poolid<=8;poolid++) {
                        if (MCrelayISO.SLFreePool[poolid]==-1) {
                                System.out.println("Conexion SibLink asignada a estacion: "+poolid+") IP Remota: "+s.getRemoteSocketAddress().toString());
                                MCrelayISO.SLFreePool[poolid]=poolid;MCrelayISO.SLconnID=poolid; break;} }


                if ( MCrelayISO.SLconnID < 8) { 
			BufferedInputStream din=new BufferedInputStream(s.getInputStream(),2048);
	                BufferedOutputStream dout=new BufferedOutputStream(s.getOutputStream(),2048);
	
			MCrelayISO.SLBISpool[MCrelayISO.SLconnID] = din;
	                MCrelayISO.SLBOSpool[MCrelayISO.SLconnID] = dout;
	
	                // create a new thread object
	                Thread t = new SiblinkHandlerThread(s, din, dout, MCrelayISO.SLconnID);
			...
			MCrelayISO.MuestraConn();
                 ...
                 } else { System.out.println("Conexion rechazada, maximo de 8 conexiones alcanzado...");
		          try {s.close();} catch (Exception e) {System.out.println("Exception: "+e);}
		 }


	       } catch (Exception e){ e.printStackTrace(); Mainexit=false;}
       	} // Fin de While

           try {ssa.close();} catch (Exception e) { System.out.println("Exception: "+e); } 
...
{ 
	...
	try {
				...
				s.setSoTimeout(MCrelayISO.SLECOsendtimeout); // Si Excede timeout envio ECO a siblink
                                ...
                                if ((msgi=din.read()) ==-1) {System.out.println("======> Siblink cerro la conexion ("+SLNroConn+") libero pool y cierro socket");exit=false; break;}
...
//				if (MCrelayISO.DebugTramaServidor.charAt(0) == (char) 'F' && exit) {System.out.println("Bytes disponibles en buffer in Siblink: "+ISOsize);}
                                ...
                                // si esta en EOF, salgo
                                if ((msgj=din.read())==-1) {System.out.println("======> Siblink cerro la conexion ("+SLNroConn+") libero pool y cierro socket");exit=false; break;}

                                int msgsize=msgi*256+msgj;

                                if (din.read(b,0,ISOsize)==-1) {System.out.println("======> Siblink cerro la conexion ("+SLNroConn+") libero pool y cierro socket");exit=false; break;}
                                ...
                                if (MCrelayISO.DebugTramaServidor.charAt(0) == (char) 'F' && exit) { // INICIO LOGGINF
                                    str="";
                                    for (int msgpos=0;msgpos<msgsize;msgpos++) {str=str+(char) b[msgpos];}
                                        MCrelayISO.Bufferedfw.write("\n\n ========== Respuesta desde Siblink ("+SLNroConn+") IP:"+s.getRemoteSocketAddress().toString()+"  =========\n<< ");
			...
			if (MCrelayISO.ConnPool[SLNroConn]!=-1 && exit ) { //SOLO ESCRIBO SI EL OTRO EXTREMO ESTA VIVO
			  ...
			  MCrelayISO.BOSpool[MCrelayISO.ConnPool[SLNroConn]].write(b,0,msgsize); 
...
//				System.out.println("$$$> Excepcion:"+MCrelayISO.ConnPool[SLNroConn]+"--:"+e);
				if (EnvioEco(s,din,dout,SLNroConn) != 0) {exit=false; MCrelayISO.SLFreePool[SLNroConn]=-1; MCrelayISO.MuestraConn();}
						}
		} //FIN While
...
//
public int EnvioEco(Socket s, BufferedInputStream din, BufferedOutputStream dout, int NroConnE) {
	int i=0;

       //      Escribo ECO .....
	try {	
		if (MCrelayISO.LOGecos.charAt(0) == (char) 'S') System.out.println("Enviando ISO ECO a estacion SibLink ("+NroConnE+")"); 
                i=str2.length();
				...
				System.out.println("No Pude enviar ECO a estacion ("+NroConnE+") Cerrando conexion"); 
			               try {
       				         din.close();
                			 dout.close();
                			 s.close();
					 return 2;
                			   } catch (Exception ee) { System.out.println("No pude cerrar el socket de la estacion: ("+NroConnE+" )"); return 2; } 
	...
	try {
		...
		if (MCrelayISO.LOGecos.charAt(0) == (char) 'S') System.out.println("Respuesta ISO ECO de estacion ("+NroConnE+")Recibida.. "); 
  			} catch (Exception e) { 
                		System.out.println("Respuesta ECO de estacion ("+NroConnE+") no recibida.. Cerrando conexion  "); 
                                       try {
                                          din.close();
                                          dout.close();
                                          s.close();
...
class ClienteHandler extends Thread
{
	...
	//
	   ...
	   while (MCrelayISO.SLconnID < 7 ) {
		try { Thread.sleep(10000); System.out.println("Esperandoo Inicio de Siblink (10 Segs..)");
		 } catch (InterruptedException e) {System.out.println("Esperando Inicio de Siblink (10 Segs..)");}
		} 
	}
        ...
        while (Mainexit)  {
            ...
            {
                Socket s=ssb.accept();
                System.out.println("======> conexion de Cliente aceptada...: ("+MCrelayISO.ClienteID+") IP Remota: "+s.getRemoteSocketAddress().toString());

		BufferedInputStream  Cdin =new BufferedInputStream(s.getInputStream(),2048);
		BufferedOutputStream Cdout=new BufferedOutputStream(s.getOutputStream(),2048);
		...
		int IDasignado=0;
		for (int poolid=0;poolid<=8;poolid++) {
		       	if (MCrelayISO.ConnPool[poolid]==-1 && MCrelayISO.SLFreePool[poolid]!=-1) {
		 		System.out.println("======> Conexion cliente asignada a estacion: "+poolid);
				MCrelayISO.ConnPool[poolid]=MCrelayISO.ClienteID; 
                                MCrelayISO.ConnIpPool[poolid]=s.getRemoteSocketAddress().toString();
				IDasignado=poolid;break; } }

		 	
		if (IDasignado<8) { // Si habian conexiones disponibles, creo nuevo thread 
                		Thread t = new ClienteHandlerThread(s, Cdin, Cdout, MCrelayISO.ClienteID);
                		t.start();
			} else { System.out.println("======> No hay conexiones disponibles en el pool, cerrando conexion " + IDasignado + " SLconnID " + MCrelayISO.SLconnID);
				try {s.close();} catch (Exception e) {System.out.println("Exception: "+e);}	
					}
	    ...
	    } catch (Exception e) { System.out.println("Exception: "+e); Mainexit=false;}

           } // Fin de While

           try {ssb.close();} catch (Exception e) { System.out.println("Exception: "+e); }
...
class ClienteHandlerThread extends Thread
{
    ...
    // Constructor 
    public ClienteHandlerThread(Socket s, BufferedInputStream Cdin, BufferedOutputStream Cdout, int ClienteID)
    {
        this.s = s;
        this.Cdin = Cdin;
        this.Cdout = Cdout;
	...
	for (int index=0;index<8;index++) {if (MCrelayISO.ConnPool[index]==ClienteID) {i=index; break;}}
        ...
        while(exit){  //loop principal de transferencia de datos desde el cliente
				...
				// lee del stream de entrada del cliente, el primer byte
				// si esta en EOF, salgo
				if ((msgi=Cdin.read())==-1) {System.out.println("======> El extremo cerro la conexion.. libero pool y cierro socket");exit=false; break;}
...
//				if (MCrelayISO.DebugTramaCliente.charAt(0) == (char) 'F' && exit) {System.out.println("Bytes disponibles en buffer In Cliente inicio: "+ISOsize);}
				...
				// lee del stream de entrada del cliente, el segundo byte
				// si esta en EOF, salgo
				if ((msgj=Cdin.read())==-1) {System.out.println("======> El extremo cerro la conexion.. libero pool y cierro socket");exit=false; break;}

				int msgsize=msgi*256+msgj;

				// lee del stream de entrada del cliente, el Mensaje ISO 
				// si esta en EOF, salgo
				if (Cdin.read(b,0,ISOsize)==-1) {System.out.println("======> El extremo cerro la conexion.. libero pool y cierro socket");exit=false; }

				// Si estaa activo el Debug deja copia de la Trama en el archivo de log.
				if (MCrelayISO.DebugTramaCliente.charAt(0) == (char) 'F' && exit) { 
			   	    str="";
				    for (int msgpos=0;msgpos<msgsize;msgpos++) {str=str+(char) b[msgpos];}
		                    MCrelayISO.Bufferedfw.write("\n\n ========== Mensaje desde cliente ("+ClienteID+") IP remota: "+s.getRemoteSocketAddress().toString()+"  ==bytes ("+msgsize+")==\n>>");
				    MCrelayISO.Bufferedfw.write(str);
				    MCrelayISO.Bufferedfw.flush();
				} //FIN LOGGING
 ...
 //                               if (MCrelayISO.DebugTramaCliente.charAt(0) == (char) 'F' && exit) {System.out.println("Bytes disponibles en buffer In Cliente Fin: "+ISOsize);}
				...
				System.out.println("======> Conexion cerrada por el cliente ("+ClienteID+") IP Remota: "+s.getRemoteSocketAddress().toString()); exit=false;} 
		catch (IndexOutOfBoundsException e) {System.out.println("Mensaje ISO Mal formado o el cliente cerro la conexion..");}
                } //FIN While
        ...
        for (int poolid=0;poolid<9;poolid++) {
         if (MCrelayISO.ConnPool[poolid]==ClienteID) {
               System.out.println("======> Conexion cliente ("+ClienteID+") liberada del pool, estacion: "+poolid);
                          MCrelayISO.ConnPool[poolid]=-1; MCrelayISO.ConnIpPool[poolid]=null;MCrelayISO.MuestraConn();}   }
